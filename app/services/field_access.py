...
def assert_field_rep_owns_job(claims: dict, job_id: str, method: str = "GET") -> None:
    """
    Ensure the requesting field rep owns or has permission for the specified job.
    Admins/operations/accounting and full-access core team members bypass this check.

    Read-only core team (Alex Wickham) may access any job via GET/HEAD/OPTIONS,
    but is blocked from mutating endpoints (POST/PUT/PATCH/DELETE) on jobs they
    do not own — consistent with the method-aware check in app.api.auth.
    This is an independent enforcement point (defense in depth) that does NOT
    rely solely on the separate check in get_current_claims.
    ...
    # Full-access core: unrestricted on all methods, for all jobs
    if rep_name in FULL_ACCESS_CORE_NAMES:
        return

    # Read-only core (Alex Wickham): read access to any job; blocked from mutations
