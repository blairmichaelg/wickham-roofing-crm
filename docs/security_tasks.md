...
- **bcrypt Range Pin (`bcrypt>=3.2.2,<4.0.0`)**: Pinned to a safe range (`>=3.2.2,<4.0.0`) in `requirements.txt` to permit patch-level security updates while excluding breaking releases. `bcrypt >= 4.0.0` / `5.0.0` changed behavior from silently truncating passwords over 72 bytes to raising `ValueError`. Under `passlib` (which expects legacy truncation behavior during salt generation / hashing), upgrading to `>= 4.0.0` causes unhandled 500 exceptions across all authentication routes. This range must not be raised to `>= 4.0.0` without an explicit migration away from `passlib`.
