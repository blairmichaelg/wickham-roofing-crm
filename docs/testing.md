...
This guide documents the **64 test modules** (430 assertions) comprising the Wickham Roofing CRM test suite, mapping each module to the business guarantee it protects, documenting how to run the suite with coverage, and specifying the enforced threshold targets.
...
| `app/services/` | ≥ 90% |
| Rest of `app/` | ≥ 75% |
| Overall | ≥ 75% (enforced via `fail_under = 75` in `pyproject.toml`; current: **76.06%**) |
